"""ORCID identity, through the Keycloak realm the whole stack already runs.

Design note §7: **login is ORCID**, and every unit and photo this assistant
writes is attributed to a real, verifiable person. That is not a compliance
detail — a field record without a hand behind it is one nobody can defend three
years later, when somebody asks who saw that layer.

So this module answers one question per request (*is this token good, and is it
for us*), and `principal_orcid` below answers the one the tools care about:
**who is speaking**. It is read from the TOKEN and never from a field the client
filled in.

stratigraph-chatbot is a **resource server**: it never logs anybody in. A client obtains an
...
store on either side. The moment stratigraph-chatbot kept a session it would stop being
...
deployment that already runs Heriverse can point stratigraph-chatbot at the same realm
...
log = logging.getLogger("stratigraph-chatbot.auth")
...
def _env(*names: str) -> str:
    ...
    explicit stratigraph-chatbot variable wins over one inherited from Heriverse's realm
    configuration."""
    for name in names:
...
def load_settings(environ: Optional[Dict[str, str]] = None) -> OidcSettings:
    ...
    audience = _env("OIDC_AUDIENCE", "CLIENT_ID_em", "CLIENT_ID_emchatbot")
    required_scope = _env("OIDC_REQUIRED_SCOPE")
    anon = _env("EM_CHATBOT_ALLOW_ANON") in ("1", "true", "yes", "on")
